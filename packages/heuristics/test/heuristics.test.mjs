// Unit tests for heuristics.js — pure functions, no DOM, so these run with
// nothing but Node itself: `node --test` from inside packages/heuristics.
//
// This intentionally does NOT test detector.js. Detector logic needs a real
// DOM and real ad-network selectors, which is what
// apps/adsentinel/test/fixtures/test-page.html is for — load that in a
// browser with AdSentinel installed and compare against the expected counts
// printed on the page. Two different failure modes, two different tools:
// this file catches "someone changed a rule and broke an existing case,"
// the fixture page catches "a DOM selector stopped matching."

import test from "node:test";
import assert from "node:assert/strict";
import { evaluateAd, pageLooksChildDirected, FLAG_TYPES } from "../heuristics.js";
